<header class="bg-black pt-15">
    <div class="container flex h-auto p-10 mx-auto items-center w-full justify-between mb-16">
        <div class="block md:hidden self-center" id="mobile">
            <i id="bars" class="fa fa-bars text-white py-12 px-[13px] text-xl"></i>
        </div>
        <a href="home.html">
            <img src="/storage/images/logo.svg" alt="logo" width="280" class="text-white px-12">
        </a>
        <div class="text-white justify-self-end flex justify-center">
            <a ><i href="search.html" class="fas fa-search p-12 text-[20px]"></i></a>
            <a href="cart.html"><i class="fas fa-shopping-bag py-12 px-[14px] text-[20px]"></i></a>
        </div>
    </div>

    <x-navigation class="hidden md:block"/>
</header>